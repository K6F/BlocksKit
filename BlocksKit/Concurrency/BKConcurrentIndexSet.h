...
@interface BKConcurrentIndexSet : NSObject
...
/** Enumerates each index in an index set concurrently and executes the
 ...
 @param block A single-argument, void-returning code block.
 */
- (void)bk_apply:(void (^)(NSUInteger index))block __attribute((deprecated("Use bk_each")));
...
@interface NSIndexSet (BlocksKit_Concurrent)
