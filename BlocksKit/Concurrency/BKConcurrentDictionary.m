...
@interface BKConcurrentDictionary ()

@property (nonatomic, strong) NSDictionary<id, id>* dict;

@end


@implementation BKConcurrentDictionary

- (instancetype)initWithDictionary:(NSDictionary<id, id>*)dict
{
	if (self = [super init])
	{
		self.dict = dict;
...
{
	[self.dict enumerateKeysAndObjectsWithOptions:NSEnumerationConcurrent
...
{
	NSSet* passedKeys = [self.dict keysOfEntriesWithOptions:NSEnumerationConcurrent
																							passingTest:^BOOL(id  _Nonnull key, id  _Nonnull obj, BOOL * _Nonnull stop) {
																								BOOL passed = block(key, obj);
																								*stop = passed;
																								return passed;
																							}];
	id result = nil;
	...
	{
		result = [self.dict objectForKey:passedKeys.anyObject];
	}
...
{
	NSArray* keys = [[self.dict keysOfEntriesWithOptions:NSEnumerationConcurrent
																					 passingTest:^BOOL(id  _Nonnull key, id  _Nonnull obj, BOOL * _Nonnull stop) {
																						 BOOL passed = block(key, obj);
																						 return passed;
																					 }] allObjects];
	NSArray* objects = [self.dict objectsForKeys:keys notFoundMarker:[NSNull null]];
...
{
	NSMutableDictionary* result = [[NSMutableDictionary alloc] initWithCapacity:self.dict.count];
	BKLock* sync = [[BKLock alloc] init];
	[self.dict enumerateKeysAndObjectsWithOptions:NSEnumerationConcurrent
																		 usingBlock:^(id  _Nonnull key, id  _Nonnull obj, BOOL * _Nonnull stop) {
																			 id changedObject = block(key, obj);
																			 [sync exec:^{
																				 result[key] = changedObject;
																			 }];
																		 }];
	return result;
}
...
- (BOOL)bk_any:(BOOL (^)(id key, id obj))block
{
	...
	BOOL passed = (passedKeys.count != 0);
	return passed;
}
...
{
	BOOL somePassed = [self bk_any:block];
	BOOL none = !somePassed;
	return none;
}
...
{
	NSSet* nonPassedKeys = [self.dict keysOfEntriesWithOptions:NSEnumerationConcurrent
																								 passingTest:^BOOL(id  _Nonnull key, id  _Nonnull obj, BOOL * _Nonnull stop) {
																									 BOOL passed = block(key, obj);
																									 return !passed;
																								 }];
	BOOL all = (nonPassedKeys.count == 0);
	return all;
}
...
#pragma mark -
@implementation NSDictionary (BlocksKit_Concurrent)

- (BKConcurrentDictionary*)bk_concurrent
{
	BKConcurrentDictionary* result = [[BKConcurrentDictionary alloc] initWithDictionary:[self copy]];
	return result;
}
