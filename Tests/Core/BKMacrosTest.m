...
- (void)testEach {
    BK_EACH(self->_subject, self->_total += [(NSString *)obj length];);
    XCTAssertEqual(_total, (NSInteger)6, @"total length of \"122333\" is %ld", (long)_total);
}

- (void)testApply {
    BK_APPLY(self->_subject, @synchronized(self) {
        self->_total += [(NSString *)obj length];
    });
    XCTAssertEqual(_total, (NSInteger)6, @"total length of \"122333\" is %ld", (long)_total);
}

- (void)testMap {
    NSArray *transformed = BK_MAP(self->_subject, [obj substringToIndex:1]);
    NSArray *target = @[ @"1", @"2", @"3" ];
    XCTAssertEqualObjects(transformed, target, @"transformed items are %@", transformed);
}

- (void)testSelect {
    NSArray *found = BK_SELECT(self->_subject, ([obj intValue] < 300) ? YES : NO);
    NSArray *target = @[ @"1", @"22" ];
    XCTAssertEqualObjects(found, target, @"selected items are %@", found);
}

- (void)testReject {
    NSArray *left = BK_REJECT(self->_subject, ([obj intValue] > 300) ? YES : NO);
    NSArray *target = @[ @"1", @"22" ];
    XCTAssertEqualObjects(left, target, @"not rejected items are %@", left);
}

- (void)testMatch {
    id found = BK_MATCH(self->_subject, ([obj intValue] == 22) ? YES : NO);
    XCTAssertEqual(found, @"22", @"matched object is %@", found);
}

- (void)testReduce {
    NSString *concatenated = BK_REDUCE(self->_subject, @"", [a stringByAppendingString:b]);
...
- (void)testEachWithSecondArray {
    BK_EACH(self->_subject, self->_total += [(NSString *)obj length] * [(NSString *)BK_NEXT(self->_subject2) length];);
    XCTAssertEqual(_total, (NSInteger)1*1+2*2+3*3, @"product sum of arrays is %ld", (long)_total);
}

- (void)testMapWithSecondArray {
    NSArray *transformed = BK_MAP(self->_subject, NEXT(self->_subject2));
    XCTAssertEqualObjects(transformed, _subject2, @"transformed items are %@", transformed);
}

- (void)testSelectWithSecondArray {
    NSArray *found = BK_SELECT(self->_subject2, ([NEXT(self->_subject) intValue] < 300) ? YES : NO);
    NSArray *target = @[ @"a", @"bb" ];
    XCTAssertEqualObjects(found, target, @"selected items are %@", found);
}

- (void)testRejectWithSecondArray {
    NSArray *left = BK_REJECT(self->_subject2, ([NEXT(self->_subject) intValue] > 300) ? YES : NO);
    NSArray *target = @[ @"a", @"bb" ];
    XCTAssertEqualObjects(left, target, @"not rejected items are %@", left);
}

- (void)testMatchWithSecondArray {
    id found = BK_MATCH(self->_subject2, ([NEXT(self->_subject) intValue] == 22) ? YES : NO);
    XCTAssertEqual(found, @"bb", @"matched object is %@", found);
}

- (void)testReduceWithSecondArray {
    NSString *concatenated = BK_REDUCE(self->_subject, @"", [a stringByAppendingFormat:@"%@%@", b, NEXT(self->_subject2)]);
...
- (void)testReduceWithMultipleArrays {
    ...
    NSString *concatenated = BK_REDUCE(self->_subject, @"", [a stringByAppendingFormat:@"%@%@%@", b, NEXT(self->_subject2), NEXT(subject3)]);
